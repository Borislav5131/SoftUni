...
namespace _02.Warships
{
    class StartUp
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                    ...
                    colIndex >= 0 && colIndex < matrix.GetLength(1))
                {
                    ...
                    if (shipsPlayer1 == 0)
                    {
                        isWon = true;
                        Console.WriteLine(
                            $"Player Two has won the game! {countDestroyedShips} ships have been sunk in the battle.");
                        break;
                    }
                    else if (shipsPlayer2 == 0)
                    {
                        isWon = true;
                        Console.WriteLine(
                            $"Player One has won the game! {countDestroyedShips} ships have been sunk in the battle.");
                        break;
                    }
                }
            }
            ...
            {
                Console.WriteLine(
                    $"It's a draw! Player One has {shipsPlayer1} ships left. Player Two has {shipsPlayer2} ships left.");
            }
